...
(function(layui, window, factory) {
    ...
    Common.prototype.ajax = function(id, searchUrl, inputValue, isLinkage, linkageWidth, isSearch, successCallback, isReplace){
        ...
        $.ajax({
            ...
            success: (res) => {
                ...
                ajaxConfig.ajaxSuccessCall && ajaxConfig.ajaxSuccessCall();
                ajaxConfig.ajaxSuccessCall = null;
            },
            error: (err) => {
    ...
    Common.prototype.renderSelect = function(id, tips, select){
        ...
        arr.push(`<dd class="${FORM_SELECT_TIPS} ${FORM_NONE} ${arr.length === 2 ? FORM_EMPTY:''}">请输入检索关键字</dd>`);
        return arr.join('');
    }
    ...
    Common.prototype.addLabel = function(id, div, val){
        ...
        if(div.find('input') && div.find('input')[0]){
    ...
    Select4.prototype.search = function(id, val,ajaxSuccessCall){
        if(id && common.check(id)){
            ajaxs[id] = $.extend(true, {}, ajaxs[id] || ajax, {
                first: true,
                searchVal: val,
                ajaxSuccessCall: ajaxSuccessCall
            });

...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
    ...
    var $ = layui.$
        ...
        ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
            ...
            ,'<i class="layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop" style="font-size: 80px;color: green;font-weight: bold"></i>'
            ,'</div>'
            ,'{{# } }}'
            ...
            switch(events){
                ...
                case 'LAYTABLE_PRINT': //打印
                    var printWin = window.open('打印窗口', '_blank')
                        ...
                        ,'th,td{line-height: 20px; padding: 9px 15px; border: 1px solid #000; text-align: left; font-size: 12px; color: #000;}'
            ...
            layui.each(data, function(i1, item1){
                ...
                if(typeof id === 'object'){ //如果 id 参数直接为表头数据
                    ...
                    table.eachCols(id, function(i3, item3){
                        if(item3.field && (item3.type == 'normal' || item3.type == 'numbers') && !item3.hide){
                            var content = item1[item3.field];
                            if(content === undefined || content === null) content = '';
                            i1 == 0 && dataTitle.push(item3.title || '');
                            if(item3.type==='numbers'){
                                content=parseInt(i1)+1;
